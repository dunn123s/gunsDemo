...
public class ConstantsContext {
     ...
     * 获取验证码开关
     */
    public static Boolean getGoogleAuthOpen() {
        String googleAuthOpen = (String) CONSTNTS_HOLDER.get("GOOGLE_AUTH_OPEN");
        if (CommonStatus.ENABLE.getCode().equalsIgnoreCase(googleAuthOpen)) {
            return true;
        } else {
            return false;
        }
     ...
     * 获取验证码开关
     */
    public static int getLoginErrorMaxCnt() {
        String loginErrorMaxCnt = (String) CONSTNTS_HOLDER.get("LOGIN_ERROR_MAX_CNT");
        return Integer.parseInt(loginErrorMaxCnt);
    }
     ...
     * 获取管理系统名称
     */
    public static String getXKSpeakTemplate() {
        String systemName = (String) CONSTNTS_HOLDER.get("XK_SPEAK_TEMPLATE");
        if (ToolUtil.isEmpty(systemName)) {
            log.error("系统常量存在空值！常量名称：XK_SPEAK_TEMPLATE，采用默认名称：");
            return "恭喜来自{dept}的{name}完成一个新开! 好棒棒~加油, 么么哒~";
        } else {
            return systemName;
        }
    }
