...
layui.define(["jquery", "layer"], function (f) {
    ...
    var m = {
        ...
        flexible: function (n) {
            ...
            // 使用了layer的iframeAuto，并防止高度超出父页面高度，而导致弹出框按钮被遮盖
            ...
            var maxHeight = $(parent.window).height();
            var heg = layer.getChildFrame('html', index).outerHeight();
            var layero = $('#'+ doms[0] + index);
            var titHeight = layero.find(doms[1]).outerHeight() || 0;
            var btnHeight = layero.find('.'+doms[6]).outerHeight() || 0;

            var maxIframeHeight = maxHeight - titHeight - btnHeight - 50;
            if(maxIframeHeight < heg) heg = maxIframeHeight;

            var layeroHeight = heg + titHeight + btnHeight;
            layero.css({height: layeroHeight, top: (maxHeight-layeroHeight)/2});
