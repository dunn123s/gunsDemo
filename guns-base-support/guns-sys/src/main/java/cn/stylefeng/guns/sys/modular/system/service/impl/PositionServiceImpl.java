...
public class PositionServiceImpl extends ServiceImpl<PositionMapper, Position> implements PositionService {
     ...
     * 获取所有职位列表
     */
    public List<PositionResult> allList() {
        PositionParam paramCondition = new PositionParam();
        return this.baseMapper.customList(paramCondition);
    }

    @Override
    public List<Position> getPositionByUserId(Long userId) {
        Set<Long> posIds = userPosService.lambdaQuery()
                .eq(UserPos::getUserId, userId)
                .list()
                .stream()
                .map(UserPos::getPosId)
                .collect(Collectors.toSet());
        return lambdaQuery().in(Position::getPositionId, posIds).list();
    }
}
