...
public class GlobalExceptionHandler {
    ...
    public String credentials(InvalidGoogleAuthException e, Model model) {
        String username = getRequest().getParameter("username");
        LogManager.me().executeLog(LogTaskFactory.loginLog(username, "谷歌身份验证错误", getIp()));
        model.addAttribute("tips", "谷歌身份验证错误");
        return "/login.html";
    }
    ...
    public String credentials(InvalidLoginErrorLimitException e, Model model) {
        String username = getRequest().getParameter("username");
        LogManager.me().executeLog(LogTaskFactory.loginLog(username, "登录密码连续多次错误已被停用", getIp()));
        model.addAttribute("tips", "登录密码连续多次错误已被停用");
        return "/login.html";
    }
